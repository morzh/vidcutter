...
class QTimeLine(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.pointerPos is not None:
            x = int(self.clip(self.pointerTimePos, 0, self.duration)/self.getScale())
    ...
    def mousePressEvent(self, e):
        if e.button() == Qt.LeftButton:
            ...
            print('self.pointerPos', self.pointerPos, 'self.pointerTimePos', self.pointerTimePos)
...
def main():
    ...
    qtimeline = QTimeLine(2.4, 360) # Creates an 360/60 = 6 minutes long timeline
    videoSample_1 = VideoSample(2)
    videoSample_2 = VideoSample(1.5)
    qtimeline.videoSamples.append(videoSample_1)
    qtimeline.videoSamples.append(videoSample_2)
    qtimeline.show()
    app.exec_()
